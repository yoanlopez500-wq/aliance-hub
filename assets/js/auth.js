...
async function initAdminNav() {
    ...
    if (isPublicPage && isPlayer) { renderPlayerNav(nav, playerData, session); return; }
    ...
    if (session) {
        ...
        // FIX: Agregar enlace a Duelos para lideres de alianza
        var links = [
            ...
            { href: ahPath('admin/duel-manager.html'), label: '&#9876;&#65039; Duelos', minRole: 'alliance_leader', requiresAlliance: true },
            { href: ahPath('admin/reports.html'), label: '&#128680; Reportes', minRole: 'moderator' },
        ];
        ...
        nav.innerHTML = '<div class="bg-slate-900 text-white p-4"><div class="max-w-7xl mx-auto flex items-center justify-between"><a href="' + ahPath('index.html') + '" class="text-xl font-bold text-amber-400">&#9876;&#65039; Alliance Hub</a><div>' + adminBtn + '</div></div></div>';
    }
}
...
function playerLogout() { clearPlayerData(); window.location.href = ahPath('index.html'); }
